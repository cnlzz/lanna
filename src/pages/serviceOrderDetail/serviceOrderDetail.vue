<template>
	<view class="order-detail">
        <view class="title">
            <view v-if="checkItem.paid==0">待付款</view>
            <view v-if="checkItem.paid==1 && checkItem.status==0">待使用</view>
            <view v-if="checkItem.paid==1 && checkItem.status==3">已完成</view>
            <view v-if="checkItem.paid==1 && checkItem.status==4">已取消</view>
        </view>
        <view class="detail" v-if="showDetail">
            <view class="store">
               ...
               {{ checkItem.store.name }}  {{ checkItem.arrival_started_at }}
            </view>
            <view class="line"></view>
            <view class="services">
                <view class="service-item" v-for="(item,index) in checkItem.services" :key="index">
                    <view class="item-name">{{item.service_name}}</view>
                    <view class="item-price">¥{{item.mem_price/100}}</view>
                </view>
            </view>
            <view class="line"></view>
            <view class="coupon" v-if="hasCardVoucher">
                <view class="coupon-title">优惠券</view>
                <view class="coupon-name">{{checkItem.card_voucher.name}}</view>
            </view>
            <view class="total">
                <view class="total-people">合计{{checkItem.services.length}}人</view>
                <view class="total-price">¥{{ checkItem.total_amount/100 }}</view>
            </view>
        </view>
        <view class="order-code" v-if="showDetail">
            <view class="order-id">
                <view class="order-title">订单编号</view>
                <view class="id">{{checkItem.no}}</view>
            </view>
            <view class="order-id">
        ...
        <view class="btns" v-if="checkItem.evaluations_count==0">
            <view class="cancle" v-if="checkItem.paid==0" @click="cancelServiceOrder(checkItem.id)">取消订单</view>
            <view class="pay" v-if="checkItem.paid==0" @click="goPay(checkItem.id)">去支付</view>
            <!-- <view class="refund" v-if="checkItem.paid==1 && checkItem.status==0">申请退款</view> -->
            <view class="refund" v-if="checkItem.paid==1 && checkItem.status==3 && checkItem.evaluations_count==0" @click="goComment(checkItem.store_id,checkItem.id)">去评价</view>
        </view>
        <u-popup v-model="showPayPopup" mode="bottom" border-radius="24" height="450rpx" closeable="true">
            <view class="pay-box">
                <view class="pay-title">支付</view>
                <view class="amount">￥{{checkItem.total_amount}}</view>
                <view class="methods">
...
export default {
    ...
    onLoad(){
        let itemDetail=this.$Route.query.checkItem
        if(itemDetail.id){
            this.$api.getServiceOrderDetail(itemDetail.id).then((res)=>{
                this.checkItem=res.data.data
                if(JSON.stringify(this.checkItem.card_voucher)!=='{}'){
                    this.hasCardVoucher=true
                }
